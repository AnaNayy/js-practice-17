// Завдання 1
/**
 * Функція `replaceText` замінює всі входження певного слова у тексті
 * на задану фразу.
 *
...
function replaceText(word, replacement, text) {
  ...
  // Використання методу `replace` регулярного виразу для заміни
  //слова на фразу у тексті.
...
function countEmails(str) {
  ...
  // email - адрес /\b[A - Za - z0 -9._ % +-] + @[A - Za - z0 - 9. -] +\.[A - Za - z]{ 2,} \b / g.
  ...
  const hasMatch = str.match(regex);

  const countEmails = hasMatch ? hasMatch.length : 0;
...
// Завдання 5
/**
 * Функція `findWordOccurrences` знаходить всі входження заданого слова у
 * рядок з урахуванням ігнорування регістру.
 *
...
function findWordOccurrences(str, word) {
  ...
  // За допомогою циклу while створюємо ітерацію поки рядок містить збіги
  //з регулярним виразом, та змінній match присвоюємо збіги
...
function checkRegexFlags(regex) {
  ...
  const multiFlag = flags.includes("m");

  return globalFlag && multiFlag;
}
...
// Завдання 7
/**
 * Функція `replaceWordOccurrences` замінює всі входження заданого
 * слова у рядку на нове слово.
...
// Завдання 8
/**
 * Функція `checkFlags` перевіряє, які флаги використовуються в заданому
 * регулярному виразі.
...
/**
 * Функція `checkRegexMethods` перевіряє, які методи використовуються
 * в заданому регулярному виразі.
 *  regex - Регулярний вираз для перевірки.
